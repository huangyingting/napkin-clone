...
(function () {
  ...
  function adoptPreset(themeId) {
    ...
    cfg.sourceKind = "preset";
    cfg.sourceId = t.id;
    cfg.sourceName = t.name;
    pristineSig = sig(cfg);
  }
  function adoptCustom(p) {
    cfg = clone(p.cfg);
    cfg.sourceKind = "custom";
    cfg.sourceId = p.id;
    cfg.sourceName = p.name;
    pristineSig = sig(cfg);
  }
  ...
  function paintPreviews() {
    document.getElementById("stage").innerHTML = renderSlidePreview(
      "content",
      configAsTheme(cfg),
      { ratio: 16 / 9, width: 620 },
    );
    const lp = document.getElementById("livePreview");
    if (lp)
      lp.innerHTML = renderSlidePreview("title", configAsTheme(cfg), {
        ratio: 16 / 9,
        width: 248,
      });
    refreshStatus();
  ...
  function refreshStatus() {
    const ps = document.getElementById("presetStatus");
    if (ps) ps.innerHTML = statusHTML();
    const cs = document.getElementById("custStatus");
    if (cs) cs.innerHTML = statusHTML();
    renderFoot();
  }
  ...
  function presetCard(t, opts = {}) {
    const active =
      !opts.customId && cfg.sourceKind === "preset" && cfg.sourceId === t.id;
    const activeCustom =
      opts.customId &&
      cfg.sourceKind === "custom" &&
      cfg.sourceId === opts.customId;
    const isActive = active || activeCustom;
    const modified = isActive && isDirty();
    const theme = opts.themeId || t;
  ...
  const api = {
    ...
    previewHTML: () =>
      `<div class="cust-preview"><div class="preview-frame" id="livePreview"></div></div>`,
    matrixHTML: () =>
      `<div><div class="section-title" style="margin-bottom:6px">Typography</div><div class="matrix" id="matrix">${TEXT_ROLES.map(matrixRow).join("")}</div></div>`,
    ...
    paletteBand: () =>
      `<div class="palette-band">${window.PALETTE_TOKENS.map((p) => `<button class="seg" title="${p.label} — ${p.role}" data-color-path="colors.${p.token}" data-paint-self style="background:${cfg.colors[p.token]}"></button>`).join("")}</div><div class="palette-band-labels">${window.PALETTE_TOKENS.map((p) => `<span>${p.label}</span>`).join("")}</div>`,
    /** Named swatch grid: tile + label + hex per token. `opts.exclude` omits tokens. */
    paletteGrid: (opts = {}) => {
      const ex = opts.exclude || [];
      return `<div class="palette-grid">${window.PALETTE_TOKENS.filter(
        (p) => !ex.includes(p.token),
      )
        .map(
          (p) =>
            `<button class="pal-tile" data-color-path="colors.${p.token}"><span class="sw dot" style="background:${cfg.colors[p.token]}"></span><span class="meta"><b>${p.label}</b><span class="hx hex">${cfg.colors[p.token].toUpperCase()}</span></span></button>`,
        )
        .join("")}</div>`;
    },
    applyColors: (colors) => {
      Object.assign(cfg.colors, colors);
      renderCustomize();
      paintPreviews();
    },
  ...
  function renderCustomize() {
    ...
    root.querySelectorAll("select[data-role]").forEach(
      (s) =>
        (s.onchange = () => {
          cfg.roles[s.dataset.role].fontFamily = s.value;
          if (s.dataset.role === "h1") cfg.headingFontFamily = s.value;
          renderCustomize();
          paintPreviews();
        }),
    );
    root.querySelectorAll("input[data-size]").forEach(
      (i) =>
        (i.onchange = () => {
          cfg.roles[i.dataset.size].size =
            +i.value || cfg.roles[i.dataset.size].size;
          paintPreviews();
        }),
    );
    wireColorTriggers(root, cfg);
    if (window.PalettePlugin.wireBody)
      window.PalettePlugin.wireBody(root, cfg, api);
    paintPreviews();
  }
  ...
  function renderFoot() {
    ...
    if (savingMode) {
      ...
      foot.querySelector("#cancelSave").onclick = () => {
        savingMode = false;
        renderFoot();
      };
      foot.querySelector("#confirmSave").onclick = () => {
        const name = (
          foot.querySelector("#presetName").value || "Custom"
        ).trim();
      ...
      setTimeout(() => {
        const n = foot.querySelector("#presetName");
        if (n) n.focus();
      }, 0);
      return;
    }
  ...
  function showView(name) {
    document
      .getElementById("view-preset")
      .classList.toggle("on", name === "preset");
    document
      .getElementById("view-customize")
      .classList.toggle("on", name === "customize");
  ...
  window.initThemePanel = function initThemePanel(opts = {}) {
    adoptPreset(opts.themeId || "indigo");
    document.getElementById("toolbar").innerHTML = toolbarHTML("theme");
    document.getElementById("goCustomize").onclick = () =>
      showView("customize");
    document.getElementById("back").onclick = () => showView("preset");
    document.getElementById("reset").onclick = () => {
      if (cfg.sourceKind === "custom")
        adoptCustom(customPresets.find((p) => p.id === cfg.sourceId));

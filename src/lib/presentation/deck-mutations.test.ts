...
test("updatePresentationThemeOverrides patches typography, defaults, and reset branches", () => {
  ...
  const next = updatePresentationThemeOverrides(deck, {
    ...
    defaultBackground: { type: "solid", color: { value: "#f8fafc" } },
    bullet: { gap: 3 },
    connector: { color: "#334155", arrowEnd: "filled" },
    image: { fitMode: "cover" },
  ...
  assert.equal(tokenSet.defaultBackground.color.value, "#f8fafc");
  assert.equal(tokenSet.bullet.gap, 3);
  assert.equal(tokenSet.connector.arrowEnd, "filled");
  ...
  assert.equal((reset as any).design.themeOverrides, undefined);
  ...
  assert.equal(
    (preservedMetadata as any).design.themeOverrides.tokenSet,
    undefined,
  );
});
